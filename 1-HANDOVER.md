...
| **Verified against real Azure** | **No — see §8** |
...
scripts/
  ...
  Test-Cortex.ps1     Health check a deployment.
...
## 8. What is not verified, and what to do first

**Nothing has run against real Azure.** The environment this was built in had no Azure credentials and no npm registry. Everything is written to verified API shapes and tested against stubs, but the first live run will find something.

**Do these in order:**

1. `.\scripts\Deploy-Cortex.ps1 -WhatIfResources` — confirms the resource mapping without changing anything.
2. `node scripts/bootstrap.js --dry-run` — validates the content payloads. No Azure needed.
3. Deploy, then bootstrap. **Watch the Purview publish transition** — it is the least certain call in the codebase. If it is refused the script falls back to `DRAFT` and says so.
4. `npm install` has never been run — `govuk-frontend` is declared but was never fetched. The app falls back to a bundled stylesheet with the same class names, so it renders either way, but verify the vendored path works.
