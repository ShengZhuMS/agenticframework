...
Environment variables work as a fallback where the vault has no value.

```powershell
.\scripts\Start-Local.ps1 -Groups all-staff,waste-crime,analysts
```

That reads the vault with your Azure CLI sign-in. To bypass the vault entirely and use environment variables:

```powershell
$env:APIM_SERVICE_NAME = 'apim-cortex'
$env:FOUNDRY_PROJECT_ENDPOINT = 'https://res.services.ai.azure.com/api/projects/cortex'
npm start
```

There is no offline mode — local means your machine against real Azure resources.
