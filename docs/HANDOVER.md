...
| **`spawn az ENOENT` on Windows** | Any local run that needs a token dies instantly. The CLI is installed and works from the same prompt | `execFile` does not apply PATHEXT, and on Windows the CLI is `az.cmd` — there is no `az.exe`. `token.js` now tries `az.cmd`, `az.bat`, `az`. Never invisible on Linux, so it only bites locally |
