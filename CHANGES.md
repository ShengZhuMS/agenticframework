...
- Fixed live Map rendering: coordinates and circle sizes are generated from current governance domains and counts, with an empty state and degraded-source notice.
- Added ordered multi-agent tasks, per-step outputs and fail-stop handoffs. Existing schedule formats remain compatible; draft access and controls are owner-only.
- Added automatic pre-publication Foundry cloud red teaming, durable monitoring, complete passing-evidence requirements and version-pinned invocation.
- Added original Microsoft-inspired and Novo Nordisk-inspired presentation, preserving the existing default theme.
- Added separate additional-app deployment preparation with unique state containers and additive Entra redirects.
- Added reviewed, fingerprinted, resumable content reset; replaced the misleading print-only reset entry point.
